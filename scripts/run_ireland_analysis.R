...
# ------------------------------------------------------------
# 12. CRPS at each forecast horizon, by age group + aggregated
# ------------------------------------------------------------
N_total          <- dim(X_marginal)[1]
T_obs            <- dim(X_marginal)[2]
n_particles_crps <- 2000L
...
# --- Aggregated (Total) series, using C_total_mat ---
crps_total     <- coverage_total <- rep(NA_real_, H)
obs_total_series <- covid_data %>%
  filter(Date > cut_date & Date <= cut_date + horizon) %>%
  select(all_of(age_labels)) %>%
  rowSums()
for (h in seq_len(H)) {
  t_idx <- t_forecast_start + h - 1L
  if (t_idx > T_obs) next
  sim_log_tot <- log1p(C_total_mat[particle_idx, t_idx])
  obs_log_tot <- log1p(obs_total_series[h])
  crps_total[h] <- scoringRules::crps_sample(y = obs_log_tot, dat = sim_log_tot)
  q_tot <- quantile(sim_log_tot, c(0.025, 0.975))
  coverage_total[h] <- obs_log_tot >= q_tot[1] && obs_log_tot <= q_tot[2]
}
...
crps_total_df <- data.frame(
  date      = forecast_dates,
  days      = as.numeric(forecast_dates - cut_date),
  age_group = "Total",
  crps      = crps_total
)

crps_df <- bind_rows(crps_df, crps_total_df) %>%
  mutate(age_group = factor(age_group, levels = c(age_labels, "Total")))

coverage95 <- c(colMeans(coverage_mat, na.rm = TRUE), mean(coverage_total, na.rm = TRUE)) * 100
names(coverage95) <- c(age_labels, "Total")
...
age_labels_crps <- crps_df %>%
  ...
  mutate(label = sprintf("%s (CRPS = %.2f,  Cov95 = %.1f%%)",
                         age_group, mean_crps,
                         coverage95[match(age_group, names(coverage95))])) %>%
  { setNames(.$label, .$age_group) }

age_colors_total <- c(age_colors, "Total" = "black")

fig_crps <- ggplot(crps_df, aes(x = days, y = crps, colour = age_group, group = age_group)) +
  geom_line(linewidth = 0.8, data = filter(crps_df, age_group != "Total")) +
  geom_point(size = 1.8, data = filter(crps_df, age_group != "Total")) +
  geom_line(linewidth = 1.1, linetype = "dashed", data = filter(crps_df, age_group == "Total")) +
  geom_point(size = 2.0, shape = 17, data = filter(crps_df, age_group == "Total")) +
  scale_colour_manual(values = age_colors_total, name = "Age group", labels = age_labels_crps) +
  scale_x_continuous(breaks = unique(crps_df$days)) +
  labs(x = "Forecast horizon (days)", y = "CRPS (log scale)") +
  theme_bw(base_size = 11) +
  theme(
...
ggsave("figures/real_data/fig_crps.pdf", fig_crps, width = 8, height = 5)
print(fig_crps)
